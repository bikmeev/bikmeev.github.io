...
# Define order block
threshold = df['volume'].mean() + 2 * df['volume'].std() # Define threshold as mean plus two standard deviations
df['order_block'] = df['volume'] > threshold # if volume is greater than threshold, it's an order block
df['order_block'] = df['order_block'].astype(int)
